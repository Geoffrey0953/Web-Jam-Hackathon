...
async function scrapePDFContents() {
  ...
  try {
    for (const restaurant of restaurantData) {
      ...
      const response = await page.goto(restaurant.pdfURL, { waitUntil: 'networkidle2' });

      // Download the PDF as a buffer
      const pdfBuffer = await response.buffer();

      if (!pdfBuffer || pdfBuffer.length === 0) {
        console.log(`Failed to download PDF for ${restaurant.name}, skipping...`);
        restaurant.pdfSnippet = null;
        continue;
      }
    ...
    await fs.writeFile('./updated_testData_with_snippets.json', JSON.stringify(restaurantData, null, 2));
